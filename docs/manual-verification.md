...
| Presentation HUD | Start → More → Switch to presentation HUD → Pause → Resume → More/Reset | Explicit action changes the same window to approximately 248×72 effective pixels; time, state action, and More remain; Control Center is in More; Reset returns to Compact | Not run |
| HUD placement | Start on each display, drag HUD, expand/collapse, and change display topology | First HUD placement is inset from the current display's lower-right work-area edge; a dragged HUD returns to its saved position and remains visible | Not run |
| Screen capture | Capture the current display in a supported recorder while Compact/HUD is visible; toggle Hide timer from screen capture in More | The timer remains visible locally but is absent from supported captures by default; the menu toggles capture affinity, and Expanded mode has no exclusion | Not run |
